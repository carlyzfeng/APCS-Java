...
public class Grid {
  ...
  public void removeSet() {
    ...
    if(cardsInPlay <= 3 && deck.size() <= 0){
      ...
      for(int i=2; i >= 0; i--){
        if(selectedLocs.get(i).getCol() == currentCols){
          board[selectedLocs.get(i).getCol()][selectedLocs.get(i).getRow()] = null;
          cardsInPlay--;
        }
        else{
          board[selectedLocs.get(i).getCol()][selectedLocs.get(i).getRow()] = board[currentCols-1][i];
          cardsInPlay--;
        }
      }
      currentCols--;
  ...
  public void addCardToBoard(Card card) { //4 lines
